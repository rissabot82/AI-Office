...
$CertificationFile = Get-ChildItem `
    -LiteralPath "E:\AI\AI-Office\workspace\knowledge-graph\certification" `
    -Filter "CERT-KG-*.json" `
    -File `
...
if ($null -eq $CertificationFile) {
    throw "No Knowledge Graph certification record was found."
}
...
if ([string]$Certification.status -ne "certified") {
    throw "Latest Knowledge Graph certification is not certified."
}

$ManifestPath = "E:\AI\AI-Office\config\knowledge-graph\release-manifest.json"
...
if (Test-Path -LiteralPath $VersionPath -PathType Leaf) {
    ...
    $Version.release_name = "Knowledge Graph and Reasoning"
    $Version.status = "released"
    $Version.installed_at = (Get-Date).ToString("o")
    $Version.next_planned_milestone = "1.6 Multi-Agent Collaboration"
...
if (Test-Path -LiteralPath $ProjectStatusPath -PathType Leaf) {
    ...
    $Project.current_phase = "Preparing Multi-Agent Collaboration"
    $Project.current_milestone = "Knowledge Graph and Reasoning Certified"
    $Project.release_status = "operational"
    $Project.next_release = "1.6.0"
    $Project.next_milestone = "Multi-Agent Collaboration"
...
Write-Host ""
Write-Host "AI Office v1.5 Knowledge Graph and Reasoning released." `
