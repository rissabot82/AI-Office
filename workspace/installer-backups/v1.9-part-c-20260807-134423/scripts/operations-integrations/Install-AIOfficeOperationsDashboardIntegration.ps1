...
if ($Html -notmatch 'data-ai-office-operations-integrations') {
    $Host = '<div id="operations-integrations-module" data-ai-office-operations-integrations></div>'
    if ($Html -match '</main>') {
        $Html = $Html -replace '</main>', ($Host + "`r`n</main>")
        $Changed = $true
    }
    elseif ($Html -match '</body>') {
        $Html = $Html -replace '</body>', ($Host + "`r`n</body>")
        $Changed = $true
    }
}
...
if ($Changed) {
    ...
    Write-Host "Operations and Integrations dashboard integration installed: $HtmlPath" -ForegroundColor Green
}
else {
